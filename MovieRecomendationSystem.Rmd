...
genres <- as.data.frame(movies$genres, stringsAsFactors=FALSE)     #genres list

genres2 <- as.data.frame(tstrsplit(genres[,1], '[|]',
                         type.convert=TRUE), 
                         stringsAsFactors=FALSE)      # genres2 list
#tstrsplit transpose string split -- simply means to split the column using strsplit and assign the transposed result to individual columns
#genres2 has all strings seperated of genres 


colnames(genres2) <- c(1:10)  #all the strings are now put in different columns
...
#the genres2 data is now entered in the genre matrix
...
ratingmat
#write.csv(search_matrix,"searchmat.csv", row.names = FALSE)
```
...
similarity_users <- similarity(ratingmat[1:4, ], 
                               method = "cosine", 
                               which = "users")   #first four users rating similarity
as.matrix(similarity_users)
image(as.matrix(similarity_users), main = "User similarity",col=heat.colors(18))
```

In the given matrix, each row and each column corresponds to a user, and each cell corresponds to the similarity between two users. The more black the cell is, the more similar two users are. Note that the diagonal is black, since it's comparing each user with itself.
...
image(as.matrix(similarity_items), main = "Movies similarity",col=heat.colors(20))
```
...
table_views$title <- NA   #creating a new title column
...
table_views[1:6,]
table_views[!complete.cases(table_views),]
#movies[!complete.cases(movies),]
ggplot(table_views[1:6, ], aes(x = title, y = views)) +
  geom_bar(stat="identity") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
ggtitle("Most viewed Movies")
```

We see that "Forrest Gump (1994)" is the most viewed movie, exceeding the second-most-viewed "Shawshank Redemption,The (1994)" by 12 views.

### Distribution of the average movie rating   ->   TOP RATED MOVIES
...
qplot(average_ratings) + 
  ...
  ggtitle("The average movie rating")

average_ratings_relevant <- average_ratings[views_per_movie > 50] 
qplot(average_ratings_relevant) + 
  stat_bin(binwidth = 0.1) +
  ggtitle(paste("The relevant average ratings"))
```

The first image above shows the distribution of the average movie rating. The highest value is around 3.5, and there are a few movies whose rating is either 1 or 5. Probably, the reason is that these movies received a rating from a few people only, so we shouldn't take them into account. 
...
Such a selection of the most relevant data contains 378 users and 436 movies, compared to previous 668 users and 10325 movies in the total dataset.
...
qplot(average_ratings_per_user) + stat_bin(binwidth = 0.1) +
  ggtitle("The average rating per user")
```
...
#r <- as(ratings_movies, "realRatingMatrix")
...
with pearson distance, so it's the best-performing technique.

The UBCF with pearson distance is still the top model. Depending on what is the main purpose of the system, an appropriate number of items to recommend should be defined.
...
Based on the ROC curve's plot, the k having the biggest AUC is 40. Another good candidate is 30, but it can never have a high TPR. This means that, even if we set a very high n value, the algorithm won't be able to recommend a big percentage of items that the user liked. The IBCF with k = 5 recommends only a few items similar to the purchases. Therefore, it can't be used to recommend many items.
...
I have created a web application for the recommender system using the Shiny package in R, as shown in Figure 1. The web application is hosted on shinyapps.io and can be found at 
